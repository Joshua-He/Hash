...
void fileRead(char array[100][500], char filename[200], int &count);
...
void studentGenerator(Node** &hashTable, int &size, int studentCount, int &studentID, char firstFile[200], char secondFile[200]) {
  ...
  cout << "The students are generated and added to hash." << endl;
}
...
void fileRead(char array[100][500], char filename[200], int &count){
  ...
  if(infile.is_open()){
    ...
    count = -1;   // if the file can not be opened (not exist). 
  }
}
